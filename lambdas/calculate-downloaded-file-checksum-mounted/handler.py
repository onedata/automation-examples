...
) -> Union[AtmJobBatchResponse[JobResults], AtmException]:
...
def build_file_path(job: Job) -> str:
    return f'{MOUNT_POINT}/{job.args["downloadInfo"]["destinationPath"]}'


def build_job_results(job: Job, checksum: Optional[str]) -> JobResults:
    return {
        "result": {
            "filePath": job.args["downloadInfo"]["destinationPath"],
            "algorithm": job.ctx["config"]["algorithm"],
            "checksum": checksum,
        }
    }
